...
import { createUiReadModelCache } from "../runtime/ui-read-model-cache";
...
const HTML_SNAPSHOT_CACHE_TTL_MS = 30_000;
const HTML_LIVE_SESSIONS_CACHE_TTL_MS = Math.max(POLLING_INTERVALS_MS.sessionsList, 15_000);
...
const uiReadModelCache = createUiReadModelCache({
  snapshotPath: SNAPSHOT_PATH,
  projectsPath: join(process.cwd(), "runtime", "projects.json"),
  tasksPath: join(process.cwd(), "runtime", "tasks.json"),
  budgetPolicyPath: join(process.cwd(), "runtime", "budgets.json"),
  snapshotCacheTtlMs: HTML_SNAPSHOT_CACHE_TTL_MS,
  liveSessionsCacheTtlMs: HTML_LIVE_SESSIONS_CACHE_TTL_MS,
  readonlyMode: READONLY_MODE,
  getReadonlySnapshotToolClient: () => readonlySnapshotToolClient,
  readSnapshotRaw,
  buildDefaultSnapshot: defaultSnapshot,
...
async function readReadModelSourceStamp(): Promise<string> {
  return uiReadModelCache.readReadModelSourceStamp();
}

async function readReadModelSnapshot(): Promise<ReadModelSnapshot> {
  return uiReadModelCache.readReadModelSnapshot();
}

async function readReadModelSnapshotWithLiveSessions(toolClient: ToolClient): Promise<ReadModelSnapshot> {
  return uiReadModelCache.readReadModelSnapshotWithLiveSessions(toolClient);
}
...
async function renderHtml(
  ...
  const taskExecutionChainCards =
    needsTaskCertainty || needsCollaborationThreads
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    /* Final dark-theme pass: force nested utility surfaces onto dark panels so text stays readable. */
    body[data-ui-theme-resolved="dark"] .overview-kpi-card,
    body[data-ui-theme-resolved="dark"] .overview-action-item,
    body[data-ui-theme-resolved="dark"] .overview-task-strip,
    body[data-ui-theme-resolved="dark"] .status-chip,
    ...
    /* Final-final dark pass for stubborn white utility frames and low-contrast text. */
    ...
    body[data-ui-theme-resolved="dark"] .file-token-input,
    body[data-ui-theme-resolved="dark"] .file-editor-textarea {
      background: linear-gradient(180deg, rgba(13, 24, 36, 0.99), rgba(17, 31, 46, 0.97)) !important;
      border-color: rgba(132, 164, 201, 0.22) !important;
      box-shadow: 0 14px 30px rgba(2, 8, 14, 0.24) !important;
    }
    body[data-ui-theme-resolved="dark"] #global-visibility-card .signal-gauge,
    body[data-ui-theme-resolved="dark"] #global-visibility-card .signal-gauge-core,
    body[data-ui-theme-resolved="dark"] .quota-track,
    body[data-ui-theme-resolved="dark"] .summary-track,
    body[data-ui-theme-resolved="dark"] .bar-track {
    ...
    body[data-ui-theme-resolved="dark"] #overview-secondary-shell .exec-title,
    body[data-ui-theme-resolved="dark"] #overview-secondary-shell .exec-metric,
    body[data-ui-theme-resolved="dark"] #overview-secondary-shell .meta,
    body[data-ui-theme-resolved="dark"] #global-visibility-card .signal-gauge-head span,
    body[data-ui-theme-resolved="dark"] #global-visibility-card .signal-gauge-core strong,
    body[data-ui-theme-resolved="dark"] #global-visibility-card .signal-gauge-meta small,
    body[data-ui-theme-resolved="dark"] #global-visibility-card .signal-gauge-meta a,
    body[data-ui-theme-resolved="dark"] #global-visibility-card .summary-gauge-card span,
    body[data-ui-theme-resolved="dark"] #global-visibility-card .summary-gauge-card strong,
    body[data-ui-theme-resolved="dark"] #usage-pulse .usage-chip strong,
    body[data-ui-theme-resolved="dark"] .quota-label,
    body[data-ui-theme-resolved="dark"] .quota-value,
    body[data-ui-theme-resolved="dark"] .quota-foot,
...
function resolveEditableAgentScopesFromConfig(input: unknown, options?: { enforceAllowedRoots?: boolean }): EditableAgentScope[] {
  const root = asObject(input);
  const agents = asObject(root?.agents);
  const list = asArray(agents?.list);
  ...
  for (const item of list) {
    ...
    if (enforceAllowedRoots && !isEditableWorkspaceRootAllowed(workspaceRoot)) continue;
    output.push({
      agentId: rawId,
      facetKey,
...
export function resolveEditableAgentScopesFromConfigForSmoke(input: unknown): EditableAgentScope[] {
  return resolveEditableAgentScopesFromConfig(input, { enforceAllowedRoots: false });
}
