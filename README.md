...
## 这个 fork 具体改了什么
- 修复了 `总览` 和 `用量` 页的口径问题：
  - 现在它们共享同一套 runtime / subscription 真相源，不再出现首页摘要和详情页数字对不上。
- 强化了本地安全默认值：
  - 默认本地 token 鉴权
  - 默认只读
  - 默认关闭高风险写操作
- 重做了首页的信息组织：
  - 更突出今日总控态势、待处理事项、谁在忙、运行检查点和 AI 用量
  - 更适合非技术用户直接判断“现在有没有问题、先看哪里”
- 增加了更可解释的运营视图：
  - `用量` 页可以看今日 / 7 天 / 30 天的统一口径
  - 可以继续向下看智能体 / 任务 / 模型 / 会话类型的用量归因
- 增加了源文件工作台：
  - `文档` 与 `记忆` 页直接暴露活跃 agent 的文件范围，方便在控制中心里看配置、记忆和说明文件
- 保留 upstream 兼容思路：
  - 这个 fork 不是另起炉灶，而是在 upstream 基础上做本地运营、安全边界和可读性增强
